...
export function useLspbBcl() {
  ...
  const lspbItems = useMemo(() => {
    return reports.filter(
      (r) => r.diffTotalPiece !== 0 && (r.type ? r.type === "bcl" : true) && matchesActiveSite(r, site)
    );
  }, [reports, site]);
  ...
  const handleDeleteItem = (itemToDelete: BclReportItem) => {
    ...
    toast.success("Data selisih berhasil dihapus dari LSPB BCL.");
  };
  ...
  const handleExportExcel = async () => {
    ...
    const fileName = `LSPB_BCL_${new Date().toISOString().slice(0, 10)}.csv`;

    if (window.electronAPI?.saveSitemanFile) {
      try {
        const res = await window.electronAPI.saveSitemanFile("lspbBcl", fileName, csvContent);
