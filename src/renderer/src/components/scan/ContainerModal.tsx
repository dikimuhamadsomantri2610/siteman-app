        ...
        <div className="flex-1 overflow-y-auto divide-y divide-zinc-100 dark:divide-zinc-800">
          {revealedItems.length === 0 ? (
            ...
            revealedItems.map((item) => {
              ...
              const isLastScanned = item.id === lastScannedItemId;
              const eq = modalEditedQty[item.id] || { pck: item.pck, pcs: item.pcs };
              const computedTotal = eq.pck * item.coef + eq.pcs;
              return (
                <div
                  key={item.id}
                  className={`px-6 py-4 grid grid-cols-12 gap-2 items-center transition-all duration-300 ${
                    isLastScanned
                      ? 'animate-scan-glow border-y-2 border-emerald-400/60 rounded-md'
                      : isChecked
